...
async function canEnrollSubject(
	...
	const statusMap = new Map(studentStatuses.map((s) => [s.subjectId, s.status]));
...
// Agrupar por año
const curriculumByYear = studyPlan?.planSubjects.reduce(
	(acc, ps) => {
...
async function importCorrelativesFromExcel(filePath: string, careerId?: string) {
