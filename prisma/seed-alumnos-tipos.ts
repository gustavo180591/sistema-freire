...
async function main() {
	...
	for (const career of careers) {
		...
		for (let year = 1; year <= 4; year++) {
			...
			const email =
				`${tipoAlumno.firstName.toLowerCase()}.${tipoAlumno.lastName.toLowerCase()}.year${year}.${career.code.toLowerCase()}@instituto.edu`.replace(
					/[^a-z0-9.@]/g,
					''
				);
			...
			try {
				...
				console.log(
					`  ✅ Año ${year}: ${tipoAlumno.firstName} ${tipoAlumno.lastName} (${tipoAlumno.tipo}) - DNI: ${dni}`
				);
			} catch (error) {
		...
		for (const tipo of tiposAlumno) {
			const email =
				`${tipo.firstName.toLowerCase()}.${tipo.lastName.toLowerCase()}.${career.code.toLowerCase()}@instituto.edu`.replace(
					/[^a-z0-9.@]/g,
					''
				);
			const dni = `${Math.floor(10000000 + Math.random() * 90000000)}`;
