	...
	try {
      ...
      `,
			...
			if (tableDependencies.length > 0) {
				console.log(
					`❌ ABORTANDO: ${enumName} tiene ${tableDependencies.length} dependencias de tablas/columnas:`
				);
			...
			console.log(
				`✅ ${enumName} no tiene dependencias de tablas/columnas (puede eliminarse safely)`
			);
		}
