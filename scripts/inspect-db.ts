    ...
    `;
		...
		try {
			const migrations = await prisma.$queryRawUnsafe<
				Array<{ migration_name: string; started_at: Date }>
		...
		for (const model of models) {
			try {
				...
				console.log(
					`  ${model.name} (${model.table}): ${exists ? 'EXISTS' : 'NOT IN SCHEMA'} - ${count} registros`
				);
			} catch (error: any) {
		...
		try {
			const students = await prisma.$queryRawUnsafe<
				Array<{ id: string; firstName: string; lastName: string; dni: string; status: string }>
			>(`SELECT id, "firstName", "lastName", dni, status FROM "students" LIMIT 3`);
			console.log('Students:');
			students.forEach((s) =>
				console.log(`  - ${s.firstName} ${s.lastName} (${s.dni}) - ${s.status}`)
			);
		} catch (error: any) {
		...
		try {
			...
			attendance.forEach((a) =>
				console.log(
					`  - ${a.id} | ${a.subjectId} | ${a.classDate} | commissionId: ${a.commissionId}`
				)
			);
		} catch (error: any) {
