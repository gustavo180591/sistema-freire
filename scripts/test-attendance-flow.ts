...
} from '../src/lib/server/academic/plan-logic';
...
async function testAttendanceFlow() {
	...
	try {
		...
		if (students === 0 || subjects === 0 || teachers === 0) {
			...
			console.log(
				'   Para ejecutar pruebas funcionales, primero se deben crear datos de prueba.\n'
			);
			return;
		}
		...
		if (finalStatus) {
			...
			console.log(
				`   ✅ Cálculo final: ${finalStatus.attendancePercent}% (${finalStatus.regularityStatus})`
			);
		}
		console.log('');
