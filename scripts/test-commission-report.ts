...
async function testCommissionReport(commissionId: string) {
	...
	try {
		...
		console.log(
			`   - Docente: ${commission.teacher ? `${commission.teacher.lastName}, ${commission.teacher.firstName}` : 'No asignado'}`
		);
		...
		studentStats.forEach((stats, studentId) => {
			...
			console.log(
				`     Asistencia comisión: ${stats.commissionPercent}% (${stats.commissionPresent}/${stats.commissionAttendance})`
			);
		});
		console.log('');
		...
		const regularCount = Array.from(studentStats.values()).filter(
			(s) => s.regularityStatus === 'REGULAR'
		).length;
		const freeCount = totalStudents - regularCount;
		const criticalCount = Array.from(studentStats.values()).filter(
			(s) => s.attendancePercent < 75
		).length;
		const avgAttendance =
			totalStudents > 0
				? Array.from(studentStats.values()).reduce((sum, s) => sum + s.attendancePercent, 0) /
					totalStudents
				: 0;
		...
		const criticalStudents = Array.from(studentStats.values()).filter(
			(s) => s.attendancePercent < 75
		);
