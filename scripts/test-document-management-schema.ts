...
async function testDocumentManagementSchema() {
  ...
  try {
    ...
    const categories = ['ACADEMIC', 'FINANCIAL', 'ADMINISTRATIVE', 'LEGAL', 'MEDICAL', 'CERTIFICATE', 'CONTRACT', 'OTHER'];
    ...
    if (accessLogWithUser?.user?.id === testUserId) {
      console.log(`✅ DocumentAccessLog -> User relationship validated: ${accessLogWithUser.user.email}\n`);
    } else {
    ...
    if (deletedDocument?.status === 'DELETED' && deletedDocument.deletedAt) {
      console.log(`✅ Soft delete validated: status=${deletedDocument.status}, deletedAt=${deletedDocument.deletedAt.toISOString()}\n`);
    } else {
    ...
    if (documentsBySubType.length > 0) {
      console.log(`✅ Found ${documentsBySubType.length} document(s) with STUDY_CERTIFICATE subType\n`);
    } else {
    ...
    console.log('✅ Cleanup will delete access logs first, then documents (due to ON DELETE RESTRICT)\n');
