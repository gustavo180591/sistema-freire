...
function checkFileContent(filePath: string, patterns: string[]): ValidationResult {
	...
	return {
		success: allFound,
		message: allFound ? `✅ All patterns found in ${filePath}` : `❌ Some patterns missing in ${filePath}`,
		details
	};
}
...
function checkForbiddenPatterns(filePath: string, forbiddenPatterns: string[]): ValidationResult {
	...
	return {
		success: noneFound,
		message: noneFound ? `✅ No forbidden patterns in ${filePath}` : `❌ Forbidden patterns found in ${filePath}`,
...
async function testDocumentManagementUI() {
	...
	results.push(checkFileExists('src/lib/components/document-management/DocumentDetailModal.svelte'));
	...
	results.push(checkFileContent('src/routes/(app)/documentos/+page.server.ts', [
		'locals',
		'locals.user',
		'redirect',
		'/login'
	...
	results.push(checkFileContent('src/routes/(app)/documentos/+page.svelte', [
		'fetchDocuments',
		'handleUpload',
		'handleDownload',
		'handleDelete',
	...
	results.push(checkFileContent('src/lib/components/document-management/DocumentUploadForm.svelte', [
		'ownerType',
		'ownerId',
		'category',
		'subType',
	...
	results.push(checkFileContent('src/lib/components/document-management/DocumentList.svelte', [
		'onView',
		'onDownload',
		'onDelete',
		'onRestore',
	...
	results.push(checkFileContent('src/lib/components/document-management/DocumentFilters.svelte', [
		'category',
		'subType',
		'visibility',
		'status',
	...
	results.push(checkFileContent('src/lib/components/document-management/DocumentDetailModal.svelte', [
		'onDownload',
		'onDelete',
		'onRestore',
		'formatFileSize',
	...
	const forbiddenPatterns = ['$queryRaw', '$executeRaw', '@ts-ignore', '@ts-except-error', ': any', 'as any'];
	
	results.push(checkForbiddenPatterns('src/routes/(app)/documentos/+page.svelte', forbiddenPatterns));
	results.push(checkForbiddenPatterns('src/lib/components/document-management/DocumentUploadForm.svelte', forbiddenPatterns));
	results.push(checkForbiddenPatterns('src/lib/components/document-management/DocumentList.svelte', forbiddenPatterns));
	results.push(checkForbiddenPatterns('src/lib/components/document-management/DocumentFilters.svelte', forbiddenPatterns));
	results.push(checkForbiddenPatterns('src/lib/components/document-management/DocumentDetailModal.svelte', forbiddenPatterns));
	...
	const pageContent = readFileSync(join(projectRoot, 'src/routes/(app)/documentos/+page.svelte'), 'utf-8');
	const hasStorageUrl = pageContent.includes('storage/private') && (pageContent.includes('href=') || pageContent.includes('src='));
	
	results.push({
		success: !hasStorageUrl,
		message: !hasStorageUrl ? '✅ storage/private not used as URL' : '❌ storage/private used as URL'
	});
