...
try {
	...
	if (!testContains(pageContent, 'Datos Personales', 'Contiene sección de datos personales'))
		allPassed = false;

	// 4. contiene sección de sedes/localidades
	if (!testContains(pageContent, 'Sedes Habilitadas', 'Contiene sección de sedes/localidades'))
		allPassed = false;
	...
	if (!testNotContains(pageContent, 'name="locality"', 'No existe name="locality"'))
		allPassed = false;
	...
	if (!testContains(pageContent, 'name="locationIds"', 'Existe name="locationIds"'))
		allPassed = false;
	...
	if (!testContains(serverContent, 'updateLocations:', 'Existe acción updateLocations'))
		allPassed = false;

	// 15. el server carga locationPermissions
	if (!testContains(serverContent, 'locationPermissions', 'El server carga locationPermissions'))
		allPassed = false;

	// 16. el server carga Location
	if (!testContains(serverContent, 'prisma.location', 'El server carga Location'))
		allPassed = false;
	...
	if (!testContains(pageContent, 'checked={data.user.roles', 'Roles actuales usan checked'))
		allPassed = false;
	...
	if (
		!testContains(
			pageContent,
			'checked={data.user.locationPermissions',
			'Sedes actuales usan checked'
		)
	)
		allPassed = false;

	// 20. no se usa Prisma en el componente Svelte
	if (!testNotContains(pageContent, 'prisma', 'No se usa Prisma en el componente Svelte'))
		allPassed = false;
	...
	if (
		!testContains(
			pageContent,
			'bg-slate-900/70',
			'Modo claro/oscuro sigue referenciado correctamente'
		)
	)
		allPassed = false;
	...
	const updateUserForm = pageContent.substring(
		pageContent.indexOf('action="?/updateUser"'),
		pageContent.indexOf('action="?/updateUser"') + 500
	);
	for (const field of invalidFields) {
		if (updateUserForm.includes(`name="${field}"`)) {
			console.log(
				`✓ No se inventan campos fuera del schema de User (campo ${field} no está en updateUser)`
			);
		}
	}
