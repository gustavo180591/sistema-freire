...
async function verifyPreRecoveryState() {
  ...
  try {
    ...
    for (const enumName of ['PaymentAgreementChargeRelationType', 'PaymentAgreementEventType', 'PaymentAgreementInstallmentStatus', 'PaymentAgreementStatus']) {
      ...
      if (dependencies.length > 0) {
        console.log(`❌ ABORTANDO: ${enumName} tiene ${dependencies.length} dependencias de tablas/columnas:`);
    ...
    console.log('✅ Todos los enums huérfanos no tienen dependencias de tablas/columnas (pueden eliminarse safely)');
