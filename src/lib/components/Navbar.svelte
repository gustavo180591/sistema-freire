...
<nav
	class="fixed top-0 right-0 left-0 z-50 h-16 border-b border-slate-800 bg-slate-900/95 backdrop-blur-md"
>
		...
		<div class="flex h-full items-center justify-between">
			...
			<div class="flex items-center gap-6">
				...
				{#if currentSection()}
					...
					<span class="hidden text-sm font-medium text-slate-300 md:block">{currentSection()}</span>
				{/if}
			</div>
			...
			<div class="flex items-center gap-4">
				{#if user}
					...
					<div class="user-menu relative">
						...
						>
							<div
								class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-700 text-sm font-medium text-white"
							>
								{user.firstName[0]}{user.lastName[0]}
							</div>
							<span class="hidden text-sm font-medium text-white lg:block">{user.firstName}</span>
							<svg
						...
						{#if userMenuOpen}
							<div
								class="absolute top-full right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-900/95 shadow-2xl shadow-black/20 backdrop-blur-sm"
							>
								<!-- User Info Header -->
								...
								<div class="p-2">
									<a
										...
										class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-300 transition-all hover:bg-slate-800/80 hover:text-white"
									>
										<svg
											class="h-4 w-4 text-slate-400 transition-colors group-hover:text-indigo-400"
											fill="none"
											stroke="currentColor"
											viewBox="0 0 24 24"
										>
									...
									<button
										onclick={logout}
										class="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-red-400 transition-all hover:bg-red-950/20"
									>
										<svg
											class="h-4 w-4 text-red-400/70 transition-colors group-hover:text-red-400"
											fill="none"
											stroke="currentColor"
											viewBox="0 0 24 24"
										>
