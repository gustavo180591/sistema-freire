...
export type FinancialBlockType =
	| 'ENROLLMENT'
	| 'EXAM'
	| 'COURSE'
	| 'CERTIFICATE'
...
export class FinancialService {
	...
	private async createChargeInternal(
		...
		if (benefitCalculation.benefitType === 'SCHOLARSHIP' && scholarshipPercentage) {
			for (const scholarship of scholarships) {
				if (scholarship.maxMonthlyAmount) {
					...
					if (
						DecimalHelpers.isLessThan(remaining, DecimalHelpers.zero()) ||
						remaining.equals(DecimalHelpers.zero())
					) {
		...
		const finalAmount = DecimalHelpers.subtract(
			input.amount,
			DecimalHelpers.add(scholarshipAmount, discountAmount)
		);
		...
		for (const scholarship of scholarships) {
			if (
				scholarship.applicableTo.includes(concept.code) ||
				scholarship.applicableTo.includes('*')
			) {
		...
		for (const input of inputs) {
			try {
				...
				if (existingCharge) {
					errors.push({
						input,
						error: 'Ya existe una cuota para este alumno, concepto, período y ciclo lectivo'
					});
					continue;
				}
	 ...
	 */
	async applyScholarshipToCharge(
		chargeId: string,
		...
		const roleCodes = userRoles.map((ur) => ur.role.code);
		const hasPermissionResult = await hasPermission(roleCodes[0] || '', 'STUDENT_CHARGE', 'update');
		if (!hasPermissionResult) {
		...
		if (scholarship.maxMonthlyAmount) {
			const remaining = DecimalHelpers.subtract(
				scholarship.maxMonthlyAmount,
				scholarship.appliedAmount
			);
	...
	async applyDiscountToCharge(
		...
		if (
			!discount.applicableTo.includes(charge.concept.code) &&
			!discount.applicableTo.includes('*')
		) {
	...
	async evaluateFinancialBlocks(
		...
		const blockOverdueAmount = rules.blockOverdueAmount
			? new Decimal(rules.blockOverdueAmount)
			: new Decimal(0);
		...
		if (blockOnOverdue && debtSummary.overdueDebt.gt(0)) {
			...
			if (overdueCharges.length > 0) {
				...
				if (overdueDays > graceDays) {
					...
					if (
						debtSummary.overdueDebt.gte(blockOverdueAmount) ||
						debtSummary.overdueCharges >= blockOverdueCharges
					) {
						blockType = 'ALL';
					} else {
	...
	async registerPayment(input: PaymentInput): Promise<PaymentResult> {
		...
		const roleCodes = userRoles.map((ur) => ur.role.code);
		const hasPermissionResult = await hasPermission(roleCodes[0] || '', 'PAYMENT', 'create');
		if (!hasPermissionResult) {
	...
	private async allocatePaymentInternal(
		...
		for (const charge of sortedCharges) {
			if (
				DecimalHelpers.isLessThan(remainingAmount, DecimalHelpers.zero()) ||
				remainingAmount.equals(DecimalHelpers.zero())
			) {
				break;
			}
			...
			if (
				DecimalHelpers.isLessThan(chargeBalance, DecimalHelpers.zero()) ||
				chargeBalance.equals(DecimalHelpers.zero())
			) {
				continue;
			}
	...
	async cancelPayment(paymentId: string, reason: string, userId: string): Promise<void> {
		...
		const roleCodes = userRoles.map((ur) => ur.role.code);
		const hasPermissionResult = await hasPermission(roleCodes[0] || '', 'PAYMENT', 'delete');
		if (!hasPermissionResult) {
	...
	async reprintReceipt(params: { receiptId: string; userId: string }): Promise<Receipt> {
	...
	async getFinancialDashboardMetrics(): Promise<{
		...
		const totalPending = pendingChargesForTotal.reduce((sum, charge) => {
			return DecimalHelpers.add(
				sum,
				DecimalHelpers.subtract(charge.finalAmount, charge.paidAmount)
			);
		}, DecimalHelpers.zero());
	...
	async getStudentFinancialReport(studentId: string): Promise<{
		...
		const totalCharges = charges.reduce(
			(sum, charge) => DecimalHelpers.add(sum, charge.finalAmount),
			DecimalHelpers.zero()
		);
		const totalPaid = charges.reduce(
			(sum, charge) => DecimalHelpers.add(sum, charge.paidAmount),
			DecimalHelpers.zero()
		);
		...
		const overdueCharges = charges.filter(
			(charge) =>
				charge.dueDate && new Date(charge.dueDate) < new Date() && charge.status !== 'PAID'
		);
		const overdueDebt = overdueCharges.reduce(
			(sum, charge) =>
				DecimalHelpers.add(sum, DecimalHelpers.subtract(charge.finalAmount, charge.paidAmount)),
			DecimalHelpers.zero()
		);
	...
	async getPeriodFinancialReport(filters: { startDate?: Date; endDate?: Date }): Promise<{
		...
		const paymentsByMethod = paymentsByMethodResult.reduce(
			(acc, { method, _count }) => {
				acc[method] = _count;
				return acc;
			},
		...
		const receiptsByStatus = receiptsByStatusResult.reduce(
			(acc, { status, _count }) => {
				acc[status] = _count;
				return acc;
			},
	...
	async getStudentFinancialStatusWithAgreements(studentId: string) {
		...
		const integratedReport =
			await paymentAgreementService.getStudentIntegratedDebtReport(studentId);

		return {
			...originalStatus,
			agreementDebtSummary: {
				originalDebtTotal: integratedReport.originalDebtTotal,
				originalDebtCoveredByActiveAgreements:
					integratedReport.originalDebtCoveredByActiveAgreements,
