...
export async function getAcademicReport(): Promise<AcademicReportResult> {
	...
	const regularCount = rows.filter((row) => row.regularityStatus === 'REGULAR').length;

	const libreCount = rows.filter((row) => row.regularityStatus === 'LIBRE').length;

	const riskStudents = rows.filter((row) => row.attendancePercent < RISK_THRESHOLD).length;
