...
export async function getOfficialReport(): Promise<OfficialReportResult> {
	const [gradesCount, studentsCount, enrollmentsCount, latestTerm] = await Promise.all([
		prisma.grade.count(),
	...
	const period = latestTerm ? `${latestTerm.name} ${latestTerm.year}` : 'Período institucional';
