	...
	<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-8">
		<div class="flex items-start justify-between">
			...
			<div class="text-right">
				...
				<p
					class="text-4xl font-bold {data.overallAttendance >= 80
						? 'text-emerald-400'
			...
			{#each data.subjects as subject}
				<div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
					<div class="mb-4 flex items-center justify-between">
						...
						<div class="text-right">
							<p
								class="text-2xl font-bold {subject.percentage >= 80
									? 'text-emerald-400'
					...
					<div class="mb-4 h-2 w-full rounded-full bg-slate-800">
						<div
							class="h-2 rounded-full transition-all {subject.percentage >= 80
								? 'bg-emerald-400'
								: subject.percentage >= 60
									? 'bg-amber-400'
									: 'bg-red-400'}"
					...
					<div class="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
						{#each subject.entries.slice(0, 10) as entry}
							<div
								class="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-950 p-3"
							>
								<span
									class="flex h-8 w-8 items-center justify-center rounded-full {entry.present
