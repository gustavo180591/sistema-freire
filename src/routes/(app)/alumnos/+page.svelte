<script lang="ts">
	...
	function getStudentTypeBadge(student: Student) {
		if (student.isBecado) return { text: 'Becado', color: 'bg-blue-950/50 text-blue-400 border-blue-800' };
		if (student.isRecursante) return { text: 'Recursante', color: 'bg-amber-950/50 text-amber-400 border-amber-800' };
	...
	<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
		<div class="flex-1">
			<p class="mb-2 text-sm font-semibold tracking-[0.2em] text-indigo-400 uppercase">
				{#if filter}
					<a href="/carreras/{filter.careerId}" class="hover:text-indigo-300 transition">{filter.careerName}</a>
				{:else}
					Gestión Académica
				{/if}
			</p>
			...
			{#if filter}
				<div class="mt-3">
					<a href="/alumnos" class="inline-flex items-center gap-1 text-sm text-indigo-400 hover:text-indigo-300 transition">
		...
		<a
			href="/usuarios/nuevo"
			class="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 font-semibold text-white transition-all duration-200 hover:bg-indigo-500 hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900"
			...
			<div class="flex flex-1 flex-col gap-4 md:flex-row md:items-center">
				...
				<div class="relative flex-1">
					...
					<input
						...
						class="w-full rounded-xl border border-slate-700 bg-slate-950 pl-10 pr-4 py-2 text-sm text-slate-300 placeholder-slate-500 transition outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
					/>
				</div>
			</div>
			{#if hasActiveFilters()}
				<button
					...
					class="inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900"
				...
				<tbody class="divide-y divide-slate-800">
							...
							<td colspan="7" class="px-4 py-12">
								<div class="flex flex-col items-center justify-center text-center">
									...
									{#if data.students.length === 0}
										<a
											href="/usuarios/nuevo"
											class="mt-4 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900"
										>
											<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={icons.plus} />
											</svg>
									...
									{:else if hasActiveFilters()}
										<button
											onclick={clearFilters}
											class="mt-4 inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900"
										>
											<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={icons.x} />
											</svg>
					...
					{:else}
						...
						<tr class="hover:bg-slate-800/30 transition-colors">
							...
							<td class="px-4 py-3 whitespace-nowrap text-sm text-slate-300" title={student.career}>
								<div class="truncate max-w-[200px]">{student.career}</div>
							</td>
							<td class="px-4 py-3 whitespace-nowrap text-sm text-slate-300">
								{student.currentYear || 'En curso'}
							</td>
							<td class="px-4 py-3 whitespace-nowrap text-sm">
								<span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium {getStudentTypeBadge(student).color}">
									{getStudentTypeBadge(student).text}
								</span>
							</td>
							<td class="px-4 py-3 whitespace-nowrap text-sm">
								<span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium {getStatusBadgeColor(student.status)}">
							...
							<td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
								<div class="flex items-center justify-end gap-2">
									<a
										...
										class="inline-flex items-center justify-center rounded-lg p-2 text-emerald-400 transition hover:bg-emerald-950/30 hover:text-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-slate-900"
									...
									<button
										...
										class="inline-flex items-center justify-center rounded-lg p-2 text-blue-400 transition hover:bg-blue-950/30 hover:text-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-900"
										aria-label="Editar alumno"
									>
										<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={icons.pencil} />
										</svg>
									</button>
									<button
										onclick={() => deletingStudent = student}
										class="inline-flex items-center justify-center rounded-lg p-2 text-red-400 transition hover:bg-red-950/30 hover:text-red-300 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-slate-900"
