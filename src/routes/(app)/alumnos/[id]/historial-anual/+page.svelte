...
<div class="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
	...
	<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
		<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
			<div>
				...
				<p class="text-sm tracking-[0.2em] text-slate-400 uppercase">Historial Académico por Ciclo Lectivo</p>
	...
	<!-- Historial Académico -->
	<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
		<h2 class="text-lg font-semibold mb-4">Trayectoria Académica ({data.academicHistory.length})</h2>
		...
		{#if data.academicHistory.length === 0}
			<div class="text-center py-12">
				...
				{#if data.canCreate}
					<p class="text-sm text-slate-500 mt-1">Hacé click en "Agregar Año Académico" para comenzar</p>
				{/if}
			</div>
		{:else}
			<div class="space-y-4">
				{#each data.academicHistory as history}
					<div class="rounded-2xl border border-slate-800 bg-slate-800/50 p-4 hover:border-slate-700 transition">
						<div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
							<div class="flex-1">
								<div class="flex items-center gap-3 mb-2">
									<span class="text-2xl font-bold text-white">{history.year}</span>
									<span class="rounded-full px-3 py-1 text-xs font-medium border {history.statusColor}">
										{statusIcons[history.status]} {history.statusLabel}
									</span>
								</div>
								{#if history.observations}
									<p class="text-sm text-slate-400 mt-2">{history.observations}</p>
								{/if}
							...
							<div class="flex items-center gap-2">
								{#if data.canUpdate}
									<button
										onclick={() => editingHistory = { id: history.id, status: history.status, observations: history.observations || '' }}
...
{#if showCreateModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
		<div class="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-6">
			<h3 class="text-lg font-semibold mb-4">Agregar Año Académico</h3>
				...
				<div>
					...
					>
						{#each Object.entries(data.statusLabels) as [value, label]}
							<option value={value}>{statusIcons[value]} {label}</option>
						{/each}
					</select>
				</div>

				<div>
					<label for="observations" class="mb-2 block text-sm font-medium text-slate-300">Observaciones (opcional)</label>
					<textarea
						id="observations"
						name="observations"
...
{#if editingHistory}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
		<div class="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-6">
			<h3 class="text-lg font-semibold mb-4">Editar Año Académico</h3>
			...
			<form method="POST" action="?/update" use:enhance={handleUpdate} class="space-y-4">
				...
				<div>
					<label for="edit-status" class="mb-2 block text-sm font-medium text-slate-300">Estado</label>
					<select
						id="edit-status"
						name="status"
				...
				<div>
					<label for="edit-observations" class="mb-2 block text-sm font-medium text-slate-300">Observaciones</label>
					<textarea
