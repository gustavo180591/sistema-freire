...
<div class="mx-auto max-w-7xl space-y-6">
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div class="space-y-1">
			<p class="text-xs font-semibold tracking-[0.2em] text-slate-500 uppercase">Secretaría Académica</p>
			<h1 class="text-3xl font-bold tracking-tight text-white">Carreras</h1>
			<p class="text-sm text-slate-400">Administración de oferta académica, planes de estudio y trazabilidad curricular</p>
		</div>
		{#if canManageCareers()}
			<a
				href="/carreras/nueva"
							...
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">Comienza agregando la primera carrera al sistema</p>
									</div>
									{#if canManageCareers()}
										<a
											href="/carreras/nueva"
							...
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">No hay carreras que coincidan con "{search}"</p>
									</div>
									<button
										onclick={clearSearch}
					...
					{:else}
						{#each filtered as career (career.id)}
							...
							<tr class="transition-colors hover:bg-slate-800/30">
								<td class="px-4 py-3">
									<div class="min-w-0">
										<p class="text-sm font-medium text-white truncate" title={career.name}>
											{career.name}
										</p>
									</div>
								</td>
								...
								<td class="px-4 py-3 text-right">
									<div class="flex items-center justify-end gap-1">
										...
										<button
											...
											class="rounded-lg p-2 text-red-400 transition-colors hover:bg-red-500/10 hover:text-red-300 focus:outline-none focus:ring-2 focus:ring-red-500/50 disabled:text-slate-600 disabled:hover:bg-transparent disabled:cursor-not-allowed"
		...
		>
			<div class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
				<div class="flex items-start gap-4 mb-6">
					...
					<div class="flex-1">
						<h2 id="delete-modal-title" class="text-lg font-semibold text-white">Eliminar Carrera</h2>
						<p class="mt-1 text-sm text-slate-400">Esta acción no se puede deshacer. La carrera se eliminará permanentemente del sistema.</p>
					</div>
				</div>
				...
				<div class="mb-6 rounded-xl border border-slate-800 bg-slate-800/50 p-4">
					...
					<p class="text-sm text-slate-400">{deletingCareer.plans} planes • {deletingCareer.students} alumnos</p>
				</div>
