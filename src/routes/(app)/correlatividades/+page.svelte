<script lang="ts">
	...
	const filteredSubjects = $derived(
		subjects.filter((subject) => {
			const matchesCareer =
				!selectedCareerId || subject.careers.some((c) => c.id === selectedCareerId);
	...
	function getCorrelativeBadge(correlatives: string[], colorClass: string) {
		...
		const text =
			correlatives.slice(0, 2).join(', ') +
			(correlatives.length > 2 ? ` (+${correlatives.length - 2})` : '');
		return {
			text,
			class: `inline-flex items-center rounded-lg border px-2 py-1 text-xs font-medium ${colorClass} max-w-[150px] truncate`
		};
	}
	...
	function openCorrelativesModal(
		subject: (typeof subjects)[0],
		type: 'REGULAR' | 'APROBADO' | 'APROBADO_APROBAR'
	) {
...
<div class="space-y-6">
	...
	<section class="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
		<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
			<div>
				<span
					class="inline-flex items-center rounded-lg bg-indigo-500/10 px-3 py-1 text-xs font-medium text-indigo-400"
				>
					Gestión académica
				</span>
				<h1 class="mt-2 text-2xl font-semibold text-white">Correlatividades</h1>
				<p class="mt-1 text-sm text-slate-400">
					Administra los requisitos para cursar y aprobar materias del plan de estudios
				</p>
			</div>
		...
		<form class="flex flex-wrap items-center gap-3">
			<div class="flex min-w-[200px] flex-1 items-center gap-2">
				...
				<select
					...
					class="flex-1 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none"
				>
			...
			{#if hasActiveFilters}
				<a
					...
					class="rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-300 transition-all hover:bg-slate-800 focus:ring-2 focus:ring-slate-500/50 focus:outline-none"
				>
					Limpiar
				</a>
			{/if}
			...
			<div class="overflow-x-auto rounded-xl border border-slate-800">
				<table class="w-full min-w-[1400px]">
					<thead class="border-b-2 border-slate-700 bg-slate-800/50">
						<tr>
							<th
								class="border-b border-slate-700 px-4 py-4 text-left text-xs font-bold tracking-wider text-slate-300 uppercase"
								>Curso/Año</th
							>
							<th
								class="border-b border-slate-700 px-4 py-4 text-left text-xs font-bold tracking-wider text-slate-300 uppercase"
								>Unidad Curricular</th
							>
							<th
								class="border-b border-slate-700 px-4 py-4 text-left text-xs font-bold tracking-wider text-slate-300 uppercase"
								>Modalidad de Acreditación</th
							>
							<th
								class="border-b border-slate-700 px-4 py-4 text-center text-xs font-bold tracking-wider text-slate-300 uppercase"
								colspan="3">Régimen de Correlatividades</th
							>
							<th
								class="border-b border-slate-700 px-4 py-4 text-right text-xs font-bold tracking-wider text-slate-300 uppercase"
								>Acciones</th
							>
						</tr>
						<tr class="bg-slate-800/30">
							<th
								class="border-b border-slate-700 px-4 py-3 text-center text-xs font-semibold text-slate-400"
							></th>
							<th
								class="border-b border-slate-700 px-4 py-3 text-center text-xs font-semibold text-slate-400"
							></th>
							...
							<th
								class="border-b border-slate-700 px-4 py-3 text-center text-xs font-semibold text-slate-400"
								>Para cursar deberá haber regularizado</th
							>
							<th
								class="border-b border-slate-700 px-4 py-3 text-center text-xs font-semibold text-slate-400"
								>Para cursar deberá haber aprobado</th
							>
							<th
								class="border-b border-slate-700 px-4 py-3 text-center text-xs font-semibold text-slate-400"
								>Para aprobar deberá haber aprobado</th
							>
							<th
								class="border-b border-slate-700 px-4 py-3 text-center text-xs font-semibold text-slate-400"
							></th>
							...
							{#if yearSubjects.length > 0}
								<tr class="bg-indigo-950/20">
									<td colspan="7" class="px-4 py-3">
										<span
											class="inline-flex items-center gap-2 text-sm font-semibold text-indigo-300"
										>
											<svg
												class="h-4 w-4"
												fill="none"
								...
								{#each yearSubjects as subject}
									...
									<tr class="transition-colors hover:bg-slate-800/40">
										...
										<td class="px-4 py-4">
											<div class="min-w-0">
												<p class="truncate text-sm font-semibold text-white" title={subject.name}>
													{subject.name}
												</p>
												<p class="font-mono text-xs text-slate-500">{subject.code}</p>
											</div>
										</td>
										<td class="px-4 py-4">
											<span
												class="inline-flex items-center rounded-lg border px-3 py-1.5 text-xs font-semibold {accreditationBadge}"
											>
										...
										<td class="px-4 py-4 text-center">
											<span
												class={regularBadge.class}
												title={subject.correlativesRegular.join(', ')}>{regularBadge.text}</span
											>
										</td>
										<td class="px-4 py-4 text-center">
											<span
												class={cursarBadge.class}
												title={subject.correlativesAprobadoCursar.join(', ')}
												>{cursarBadge.text}</span
											>
										</td>
										<td class="px-4 py-4 text-center">
											<span
												class={aprobarBadge.class}
												title={subject.correlativesAprobadoAprobar.join(', ')}
...
{#if modalOpen && editingSubject}
	...
	>
		...
		<div
			role="document"
			class="mx-4 w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6"
		...
		>
			...
			{#if form?.success}
				<div
					class="mb-4 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-emerald-400"
				>
			...
			<form method="POST" action="?/updateAccreditationMode" use:enhance>
				...
				<div class="mb-4">
					...
					>
						<option
							value="PROMOCIONAL"
							selected={editingSubject.accreditationMode === 'PROMOCIONAL'}
						>
							Promocional
						</option>
						<option
							value="EXAMEN_FINAL"
							selected={editingSubject.accreditationMode === 'EXAMEN_FINAL'}
						>
							Examen Final
						</option>
						<option
							value="PROMOCIONAL_SIN_FINAL"
							selected={editingSubject.accreditationMode === 'PROMOCIONAL_SIN_FINAL'}
						>
...
{#if correlativesModalOpen && correlativesEditingSubject}
	...
	>
		...
		<div
			role="document"
			class="mx-4 max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-6"
			onclick={(e) => e.stopPropagation()}
		>
			...
			<div class="mb-4">
				<h3 class="mb-3 block text-sm font-medium text-slate-300">Tipo de Correlativa</h3>
				<div class="space-y-2">
					<label
						class="flex cursor-pointer items-center rounded-lg border border-slate-700 bg-slate-800/50 p-3 transition hover:bg-slate-800"
					>
						<input
							type="radio"
						...
						/>
						<span class="text-sm text-white">Para cursar deberá haber Regularizado</span>
					</label>
					<label
						class="flex cursor-pointer items-center rounded-lg border border-slate-700 bg-slate-800/50 p-3 transition hover:bg-slate-800"
					>
						<input
							...
							class="mr-3 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-900"
						/>
						<span class="text-sm text-white">Para cursar deberá haber Aprobado</span>
					</label>
					<label
						class="flex cursor-pointer items-center rounded-lg border border-slate-700 bg-slate-800/50 p-3 transition hover:bg-slate-800"
					>
						<input
							...
							class="mr-3 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-900"
						/>
						<span class="text-sm text-white">Para aprobar deberá haber Aprobado</span>
					</label>
				</div>
			</div>
			...
			<form method="POST" action="?/addCorrelative" use:enhance>
				...
				<div class="mb-4">
					<label for="searchSubjects" class="mb-2 block text-sm font-medium text-slate-300">
						Buscar Materia
					</label>
					<input
						type="text"
				...
				<div class="mb-4">
					<label for="requiredSubjectId" class="mb-2 block text-sm font-medium text-slate-300">
						Materia Requerida
					</label>
					<select
...
{#if createModalOpen}
	...
	>
		...
		<div
			role="document"
			class="mx-4 max-h-[80vh] w-full max-w-md overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-6"
			onclick={(e) => e.stopPropagation()}
		>
			...
			<form method="POST" action="?/createSubject" use:enhance>
				...
				<div class="mb-4">
					<label for="accreditationMode" class="mb-2 block text-sm font-medium text-slate-300"
						>Modalidad de Acreditación</label
					>
					<select
						id="accreditationMode"
						name="accreditationMode"
	...
	>
		...
		>
			...
			<form method="POST" action="?/updateSubject" use:enhance>
				...
				<div class="mb-4">
					<label for="editCode" class="mb-2 block text-sm font-medium text-slate-300">Código</label>
					<input
						type="text"
						id="editCode"
						name="code"
				...
				<div class="mb-4">
					<label for="editName" class="mb-2 block text-sm font-medium text-slate-300">Nombre</label>
					<input
						type="text"
						id="editName"
						name="name"
				...
				<div class="mb-4">
					<label for="editYearLevel" class="mb-2 block text-sm font-medium text-slate-300"
						>Año</label
					>
					<select
						id="editYearLevel"
				...
				<div class="mb-4">
					<label for="editAccreditationMode" class="mb-2 block text-sm font-medium text-slate-300"
						>Modalidad de Acreditación</label
					>
					<select
						...
						class="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-white focus:border-blue-500 focus:outline-none"
					>
						<option value="PROMOCIONAL" selected={crudSubject.accreditationMode === 'PROMOCIONAL'}
							>Promocional</option
						>
						<option value="EXAMEN_FINAL" selected={crudSubject.accreditationMode === 'EXAMEN_FINAL'}
							>Examen Final</option
						>
						<option
							value="PROMOCIONAL_SIN_FINAL"
							selected={crudSubject.accreditationMode === 'PROMOCIONAL_SIN_FINAL'}
							>Promocional sin examen final</option
						>
					</select>
				</div>
...
{#if deleteModalOpen && crudSubject}
	...
	>
		...
		>
			...
			<p class="mb-4 text-slate-400">
				¿Estás seguro que deseas eliminar la materia <strong>{crudSubject.name}</strong>
				({crudSubject.code})?
			</p>
		...
		>
			...
			<div class="space-y-4">
				<div>
					...
					<p class="font-medium text-white">{crudSubject.code}</p>
				</div>

				<div>
					<p class="text-sm text-slate-400">Nombre</p>
					<p class="font-medium text-white">{crudSubject.name}</p>
				</div>

				<div>
					<p class="text-sm text-slate-400">Año</p>
					<p class="font-medium text-white">{crudSubject.yearLevel}° Año</p>
				</div>

				<div>
					<p class="text-sm text-slate-400">Modalidad de Acreditación</p>
					<p class="font-medium text-white">
						{accreditationModeLabels[crudSubject.accreditationMode] ||
							crudSubject.accreditationMode}
					</p>
				</div>
...
{#if editCorrelativesModalOpen && editCorrelativesSubject}
	...
	>
		...
		>
			...
			<p class="mb-4 text-slate-400">
				{editCorrelativesSubject.name} ({editCorrelativesSubject.code})
			</p>
			...
			<div class="space-y-6">
				...
				<div>
					<h3 class="mb-2 text-sm font-medium text-emerald-400">
						Para cursar deberá haber Regularizado
					</h3>
					{#if editCorrelativesSubject.correlativesRegular.length > 0}
						<div class="space-y-2">
							{#each editCorrelativesSubject.correlativesRegular as corrName}
								<div
									class="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/50 px-4 py-2"
								>
