<script lang="ts">
	...
	const quickAccess: QuickAccessItem[] = [
		...
		},
		{
			...
			description: 'Planes de estudio y carreras',
		...
		},
		{
			...
			href: '/materias',
			icon: 'book-open',
			description: 'Materias y correlatividades',
		...
		},
		{
			...
			roles: ['SUPERADMIN', 'DIRECTOR', 'SECRETARIA', 'DOCENTE', 'PRECEPTOR', 'ALUMNO']
		},
		{
			label: 'Finanzas',
			href: '/finanzas',
			icon: 'coins',
			description: 'Pagos, recibos y configuración',
	...
	const currentTerm = $derived(data.activeTerms.length > 0 ? data.activeTerms[0] : null);
					...
					>
						...
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								...
								{:else}
									<span
										class="rounded-full bg-slate-700 px-2 py-0.5 text-xs font-medium text-slate-400"
										>Baja</span
									>
								{/if}
							</div>
