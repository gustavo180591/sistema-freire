<script lang="ts">
	...
	import { canManageDirectors } from '$lib/client/permissions';

	let { data, form }: { data: PageData; form?: any } = $props();
	let searchQuery = $state('');
	let deletingDirector = $state<Director | null>(null);
	...
	const metrics = $derived(() => {
		...
		const active = data.directors.filter((d) => d.status === 'ACTIVE').length;
	...
	// Obtener iniciales del director
...
<svelte:head>
	<title>Gestión de Directores | ISFD "PAULO FREIRE" 1117</title>
</svelte:head>
		...
		>
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-xs text-slate-400">Total Directores</p>
				</div>
			</div>
		</div>
		<div
				...
				<tbody class="divide-y divide-slate-800">
					...
					{:else}
						{#each filteredDirectors as director}
							{@const statusBadge = getStatusBadge(director.status)}
			...
			<div class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
				...
				<form
					...
					use:enhance={() => {
						return async ({ update }) => {
							await update();
							deletingDirector = null;
						};
					}}
				>
					<input type="hidden" name="id" value={deletingDirector.id} />
