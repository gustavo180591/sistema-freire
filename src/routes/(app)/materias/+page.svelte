<script lang="ts">
	...
	const hasActiveFilters = $derived(
		!!(filters.search || filters.careerId || filters.yearLevel || filters.subjectType || filters.trainingField)
	);
...
<div class="mx-auto max-w-7xl space-y-6">
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div class="space-y-1">
			...
			<p class="text-sm text-slate-400">Administración de materias, correlatividades y planes de estudio</p>
		</div>
		...
		<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-5 transition-colors hover:bg-slate-900/70">
			<div class="flex items-center gap-3">
				<div class="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-500/10">
					<svg class="h-5 w-5 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
	...
	<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
		<form class="flex flex-wrap gap-3 items-end" method="GET">
			<div class="flex-1 min-w-[200px]">
				...
				<div class="relative">
					<svg class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
					...
					<input
						...
						class="w-full rounded-xl border border-slate-700 bg-slate-950 py-2 pl-10 pr-3 text-sm text-slate-300 placeholder-slate-500 transition-all outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500"
					/>
				</div>
			</div>
			...
			<div>
				...
				<select id="careerId" name="careerId" class="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-300 transition-all outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
			...
			<div>
				...
				<select id="yearLevel" name="yearLevel" class="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-300 transition-all outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
			...
			<div>
				...
				<select id="subjectType" name="subjectType" class="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-300 transition-all outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
			...
			<div>
				...
				<select id="trainingField" name="trainingField" class="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-300 transition-all outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
			...
			<div class="flex gap-2">
				<button type="submit" class="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-950 transition-all hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-white/50">
					Filtrar
				</button>
				{#if hasActiveFilters}
					<a href="/materias" class="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-300 transition-all hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500/50">
						Limpiar
					</a>
				{/if}
			</div>
		...
		{:else if Object.keys(groupedSubjects).length === 0}
			<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-12 text-center">
				<div class="flex flex-col items-center gap-4">
					...
					<a href="/materias" class="inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-300 transition-all hover:bg-slate-700">
						<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
				...
				<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
					<div class="flex items-center justify-between mb-4">
						<div>
							...
							<p class="text-sm text-slate-400">{yearSubjects.length} materia{yearSubjects.length !== 1 ? 's' : ''}</p>
						</div>
					</div>
						...
						<table class="w-full min-w-[1000px]">
							<thead class="border-b border-slate-800 bg-slate-800/30">
								<tr>
									...
									<th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Materia</th>
									<th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Modalidad</th>
									<th class="px-3 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-400" colspan="3">Correlatividades</th>
									<th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Carreras</th>
									<th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-400">Acciones</th>
								</tr>
							...
							<tbody class="divide-y divide-slate-800">
								{#each yearSubjects as subject}
									{@const regularBadge = getCorrelativeBadge(subject.correlativesRegular, 'bg-emerald-500/10 text-emerald-400')}
									{@const cursarBadge = getCorrelativeBadge(subject.correlativesAprobadoCursar, 'bg-blue-500/10 text-blue-400')}
									{@const aprobarBadge = getCorrelativeBadge(subject.correlativesAprobadoAprobar, 'bg-purple-500/10 text-purple-400')}
									{@const accreditationBadge = accreditationModeColors[subject.accreditationMode] || 'bg-slate-500/10 text-slate-400'}
									<tr class="transition-colors hover:bg-slate-800/30">
										...
										<td class="px-4 py-3">
											<div class="min-w-0">
												<p class="text-sm font-medium text-white truncate" title={subject.name}>{subject.name}</p>
												{#if subject.isElective}
													<span class="inline-flex items-center rounded-lg bg-amber-500/10 text-amber-400 px-2 py-0.5 text-xs mt-1">Optativa</span>
												{/if}
											</div>
										</td>
										<td class="px-4 py-3">
											<span class="inline-flex items-center rounded-lg border px-2.5 py-1 text-xs font-medium {accreditationBadge}">
												{accreditationModeLabels[subject.accreditationMode] || subject.accreditationMode}
											</span>
										</td>
										<td class="px-3 py-3 text-center">
											{#if subject.correlativesRegular.length > 0}
												<span class={regularBadge.class} title={regularBadge.text}>{regularBadge.text}</span>
											{:else}
										...
										<td class="px-3 py-3 text-center">
											{#if subject.correlativesAprobadoCursar.length > 0}
												<span class={cursarBadge.class} title={cursarBadge.text}>{cursarBadge.text}</span>
											{:else}
												<span class="text-slate-600 text-xs">-</span>
											{/if}
										</td>
										<td class="px-3 py-3 text-center">
											{#if subject.correlativesAprobadoAprobar.length > 0}
												<span class={aprobarBadge.class} title={aprobarBadge.text}>{aprobarBadge.text}</span>
											{:else}
												<span class="text-slate-600 text-xs">-</span>
											{/if}
										</td>
											...
											{#if subject.careers.length > 0}
												<div class="flex flex-wrap gap-1">
													{#each subject.careers.slice(0, 2) as career}
														<span class="inline-flex items-center rounded-lg bg-slate-700 text-slate-300 px-2 py-0.5 text-xs" title={career.name}>
