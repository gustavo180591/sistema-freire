<script lang="ts">
	...
	const aprobadoAprobarCount = $derived(
		subject.correlatives.filter(
			(c: { correlativeType: string }) => c.correlativeType === 'APROBADO_APROBAR'
		).length
	);
			...
			<table class="w-full text-left">
				<thead class="border-b border-slate-800 bg-slate-900">
					<tr class="border-b border-slate-700">
						...
						<th
							rowspan="2"
							class="border-r border-slate-700 px-4 py-3 text-center text-sm font-semibold"
					...
					{#if subject.correlatives.length > 0}
						{#each subject.correlatives as corr}
							<tr class="border-b border-slate-800 last:border-none hover:bg-slate-800/50">
								<td class="border-r border-slate-700 px-4 py-4">
									<p class="font-medium">{corr.requiredSubject.name}</p>
									<p class="text-sm text-slate-400">
										{corr.requiredSubject.code} • Año {corr.requiredSubject.yearLevel}
									</p>
									{#if corr.career}
										<p class="mt-1 text-xs text-slate-500">Solo: {corr.career.code}</p>
									{/if}
								</td>
								<td class="border-r border-slate-700 px-4 py-4 text-center">
		...
		<form method="POST" action="?/addCorrelative" use:enhance class="space-y-4">
			<div class="grid gap-4 md:grid-cols-2">
				...
				<div class="space-y-2 md:col-span-2">
					<label for="requiredSubjectId" class="text-sm font-medium text-slate-300"
						>Materia Requerida</label
					>
						...
						{#each filteredSubjects as subj}
							<option value={subj.id}>
								...
								{#if subj.careerSubjects.length > 0}
									({subj.careerSubjects
										.map((cs: { career: { code: string } }) => cs.career.code)
										.join(', ')})
								{:else}
									(Común)
								{/if}
				...
				<!-- Tipo de Correlativa -->
				<div class="space-y-2">
					<label for="correlativeType" class="text-sm font-medium text-slate-300"
				...
				<div class="space-y-2">
					<label for="careerId" class="text-sm font-medium text-slate-300"
						>Aplicar solo a Carrera (opcional)</label
					>
					<select
						id="careerId"
						name="careerId"
