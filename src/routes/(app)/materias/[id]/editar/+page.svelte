<script lang="ts">
	...
	const correlativesRegularCount = $derived(
		subject.correlatives.filter((c: { correlativeType: string }) => c.correlativeType === 'REGULAR')
			.length
	);
	const correlativesAprobadoCursarCount = $derived(
		subject.correlatives.filter(
			(c: { correlativeType: string }) => c.correlativeType === 'APROBADO'
		).length
	);
	const correlativesAprobadoAprobarCount = $derived(
		subject.correlatives.filter(
			(c: { correlativeType: string }) =>
				c.correlativeType === 'LIBRE' || c.correlativeType === 'EQUIVALENCIA'
		).length
	);
</script>
...
<div class="space-y-6">
	...
	<div class="flex gap-2 border-b border-slate-800">
		<button
			class="px-4 py-3 text-sm font-medium transition {activeTab === 'general'
		...
		<button
			class="px-4 py-3 text-sm font-medium transition {activeTab === 'regimen'
				? 'border-b-2 border-white text-white'
				: 'text-slate-400 hover:text-slate-300'}"
		...
		<button
			class="px-4 py-3 text-sm font-medium transition {activeTab === 'careers'
				? 'border-b-2 border-white text-white'
				: 'text-slate-400 hover:text-slate-300'}"
		...
		<button
			class="px-4 py-3 text-sm font-medium transition {activeTab === 'teachers'
				? 'border-b-2 border-white text-white'
				: 'text-slate-400 hover:text-slate-300'}"
		...
		<section class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
			<form method="POST" action="?/update" use:enhance>
				<div class="grid gap-6 md:grid-cols-2">
					...
					<!-- Tipo de Materia -->
					<div class="space-y-2">
						<label for="subjectType" class="text-sm font-medium text-slate-300"
					...
					<!-- Campo de Formación -->
					<div class="space-y-2">
						<label for="trainingField" class="text-sm font-medium text-slate-300"
					...
					<!-- Modalidad -->
					<div class="space-y-2">
						<label for="accreditationMode" class="text-sm font-medium text-slate-300"
				...
				<div class="mt-6 flex justify-end gap-3">
					...
					<button
						type="submit"
						class="rounded-xl bg-white px-6 py-3 text-sm font-medium text-slate-950 transition hover:scale-[1.02]"
					>
						Guardar Cambios
					</button>
				</div>
		...
		<section class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
				...
				<table class="w-full text-left">
					<thead class="border-b border-slate-800 bg-slate-900">
						<tr class="border-b border-slate-700">
							<th rowspan="2" class="border-r border-slate-700 px-4 py-3 text-sm font-semibold"
								>Materia</th
							>
							<th
								colspan="2"
								class="border-r border-slate-700 px-4 py-2 text-center text-sm font-semibold"
								>Para cursar deberá</th
							>
							<th rowspan="2" class="px-4 py-3 text-center text-sm font-semibold"
								>Para aprobar deberá<br />haber Aprobado</th
							>
						</tr>
						<tr>
							<th class="border-r border-slate-700 px-4 py-2 text-center text-xs font-semibold"
								>haber Regularizado</th
							>
							<th class="border-r border-slate-700 px-4 py-2 text-center text-xs font-semibold"
								>haber Aprobado</th
							>
			...
			<div class="grid gap-4 md:grid-cols-3">
				...
				<div class="rounded-xl border border-slate-800 bg-slate-800/30 p-4">
					...
					<h3 class="mt-2 text-2xl font-bold text-purple-400">
						{correlativesAprobadoAprobarCount}
					</h3>
					...
					>
						...
						{#each teachers.filter((t: { id: string }) => !subject.teachers.some((st: { teacher: { id: string } }) => st.teacher.id === t.id)) as teacher}
							<option value={teacher.id}
								>{teacher.user.firstName} {teacher.user.lastName} - {teacher.user.email}</option
							>
						{/each}
					</select>
					<button
