<script lang="ts">
	...
	import { canManagePreceptors } from '$lib/client/permissions';

	let { data, form }: { data: PageData; form?: any } = $props();
	let searchQuery = $state('');
	let deletingPreceptor = $state<Preceptor | null>(null);
	...
	const metrics = $derived(() => {
		const total = data.preceptors.length;
		const active = data.preceptors.filter((p) => p.status === 'ACTIVE').length;
		const inactive = total - active;
		return { total, active, inactive };
	});

	// Filtrar preceptores según búsqueda
	const filteredPreceptors = $derived(
		data.preceptors.filter((preceptor: Preceptor) => {
			if (!searchQuery.trim()) return true;
			const query = searchQuery.toLowerCase();
			const fullName = `${preceptor.lastName} ${preceptor.firstName}`.toLowerCase();
			return (
				fullName.includes(query) ||
				preceptor.email.toLowerCase().includes(query)
			);
		})
	);

	// Obtener iniciales del preceptor
...
<div class="mx-auto max-w-7xl space-y-6">
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div class="space-y-1">
			...
			<h1 class="text-3xl font-bold tracking-tight text-white">Preceptores</h1>
			<p class="text-sm text-slate-400">Administración del cuerpo de preceptores del instituto</p>
		</div>
		{#if canManagePreceptors()}
			<a
		...
		<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-5 transition-colors hover:bg-slate-900/70">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-xs text-slate-400">Total Preceptores</p>
				</div>
			</div>
		</div>
		<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-5 transition-colors hover:bg-slate-900/70">
	...
	<div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
		<div class="relative">
			<label for="search" class="sr-only">Buscar preceptores</label>
				...
				<tbody class="divide-y divide-slate-800">
						...
						<tr>
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										<p class="text-sm font-medium text-white">No hay preceptores registrados</p>
										<p class="text-sm text-slate-400">Comienza agregando el primer preceptor al sistema</p>
									</div>
									{#if canManagePreceptors()}
										<a
											href="/usuarios/nuevo?type=PRECEPTOR"
							...
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">No hay preceptores que coincidan con "{searchQuery}"</p>
									</div>
									<button
										onclick={clearSearch}
					...
					{:else}
						{#each filteredPreceptors as preceptor}
							{@const statusBadge = getStatusBadge(preceptor.status)}
							<tr class="transition-colors hover:bg-slate-800/30">
								<td class="px-4 py-3">
									<div class="flex items-center gap-3">
										<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-purple-500 to-pink-600 text-sm font-semibold text-white">
											{getInitials(preceptor.firstName, preceptor.lastName)}
										</div>
										<div class="min-w-0">
											<p class="text-sm font-medium text-white truncate" title={`${preceptor.lastName}, ${preceptor.firstName}`}>
								...
								<td class="px-4 py-3">
									<p class="text-sm text-slate-300 truncate" title={preceptor.email}>{preceptor.email}</p>
								</td>
								<td class="px-4 py-3">
								...
								<td class="px-4 py-3">
									<p class="text-sm text-slate-300">{new Date(preceptor.createdAt).toLocaleDateString('es-AR')}</p>
								</td>
								<td class="px-4 py-3 text-right">
									<div class="flex items-center justify-end gap-1">
										<a
											href="/usuarios/{preceptor.id}"
										...
										<a
											href="/usuarios/{preceptor.id}/editar"
										...
										<button
											onclick={() => deletingPreceptor = preceptor}
		...
		>
			<div class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
				<div class="flex items-start gap-4 mb-6">
					...
					<div class="flex-1">
						<h2 id="delete-modal-title" class="text-lg font-semibold text-white">Eliminar Preceptor</h2>
						<p class="mt-1 text-sm text-slate-400">Esta acción no se puede deshacer. Se eliminará el rol de preceptor del usuario.</p>
					</div>
				</div>

				<div class="mb-6 rounded-xl border border-slate-800 bg-slate-800/50 p-4">
					<p class="font-medium text-white">{deletingPreceptor.lastName}, {deletingPreceptor.firstName}</p>
					<p class="text-sm text-slate-400">{deletingPreceptor.email}</p>
				</div>

				<form
					method="POST"
					action="?/deletePreceptor"
					use:enhance={() => {
						return async ({ update }) => {
							await update();
							deletingPreceptor = null;
						};
					}}
				>
					<input type="hidden" name="id" value={deletingPreceptor.id} />

					<div class="flex justify-end gap-3">
						<button
							type="button"
							onclick={() => deletingPreceptor = null}
