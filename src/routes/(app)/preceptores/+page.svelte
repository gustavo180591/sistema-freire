<script lang="ts">
	...
	const filteredPreceptors = $derived(
		data.preceptors.filter((preceptor: Preceptor) => {
			...
			return fullName.includes(query) || preceptor.email.toLowerCase().includes(query);
		})
	);
								...
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">
											Comienza agregando el primer preceptor al sistema
										</p>
									</div>
									{#if canManagePreceptors()}
										<a
							...
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">
											No hay preceptores que coincidan con "{searchQuery}"
										</p>
									</div>
									<button
										onclick={clearSearch}
						...
						{#each filteredPreceptors as preceptor}
							...
							<tr class="transition-colors hover:bg-slate-800/30">
								<td class="px-4 py-3">
									<div class="flex items-center gap-3">
										<div
											class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-purple-500 to-pink-600 text-sm font-semibold text-white"
										>
										...
										<div class="min-w-0">
											<p
												...
												title={`${preceptor.lastName}, ${preceptor.firstName}`}
											>
								...
								<td class="px-4 py-3">
									<p class="truncate text-sm text-slate-300" title={preceptor.email}>
										{preceptor.email}
									</p>
								</td>
								<td class="px-4 py-3">
								...
								<td class="px-4 py-3">
									<p class="text-sm text-slate-300">
										{new Date(preceptor.createdAt).toLocaleDateString('es-AR')}
									</p>
								</td>
			...
			<div class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
				<div class="mb-6 flex items-start gap-4">
					...
					<div class="flex-1">
						...
						<p class="mt-1 text-sm text-slate-400">
							Esta acción no se puede deshacer. Se eliminará el rol de preceptor del usuario.
						</p>
					</div>
				</div>
				...
				<div class="mb-6 rounded-xl border border-slate-800 bg-slate-800/50 p-4">
					<p class="font-medium text-white">
						{deletingPreceptor.lastName}, {deletingPreceptor.firstName}
					</p>
