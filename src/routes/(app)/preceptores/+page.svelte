...
<div class="mx-auto max-w-7xl space-y-8">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
		<div>
			...
			<h1 class="text-3xl font-bold tracking-tight">Preceptores</h1>
	...
	<div class="grid gap-4 md:grid-cols-2">
		<div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
			<div class="text-2xl font-bold text-white">{data.preceptors.length}</div>
			<div class="text-sm text-slate-400">Total Preceptores</div>
		</div>
		<div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
			<div class="text-2xl font-bold text-green-400">
				{data.preceptors.filter((p: Preceptor) => p.status === 'ACTIVE').length}
			</div>
				...
				{#if filteredPreceptors.length === 0}
					<tr>
						<td colspan="5" class="px-3 py-8 text-center text-sm text-slate-400">
							No se encontraron preceptores que coincidan con "{searchQuery}"
						</td>
					</tr>
				{:else}
					{#each filteredPreceptors as preceptor}
						<tr class="hover:bg-slate-800/30 transition-colors">
							...
							<td class="px-3 py-3 whitespace-nowrap text-sm text-slate-300 truncate" title={preceptor.email}>
								{preceptor.email}
							</td>
							<td class="px-3 py-3 whitespace-nowrap text-sm">
								<span class={getStatusColor(preceptor.status)}>
									{getStatusText(preceptor.status)}
								</span>
							</td>
		...
		<div class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
			<div class="bg-slate-900 rounded-3xl border border-slate-800 p-8 max-w-md w-full">
				<div class="flex items-center space-x-4 mb-6">
					...
					<div>
						<h2 class="text-xl font-bold text-white">Eliminar Preceptor</h2>
						<p class="text-sm text-slate-400">Esta acción no se puede deshacer</p>
					</div>
				</div>

				<div class="bg-slate-800/50 rounded-2xl p-4 mb-6">
					<p class="text-white font-medium">{deletingPreceptor.lastName}, {deletingPreceptor.firstName}</p>
					<p class="text-sm text-slate-400">Email: {deletingPreceptor.email}</p>
				</div>
