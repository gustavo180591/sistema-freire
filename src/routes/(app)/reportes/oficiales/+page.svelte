<script lang="ts">
	let { data } = $props();

	const documents = $derived(data?.documents ?? []);
</script>

<svelte:head>
	<title>Reportes oficiales | Instituto Paulo Freire</title>
	<meta
		name="description"
		content="Actas, libro matriz, nóminas y documentación oficial institucional"
	/>
</svelte:head>

<div class="space-y-8">
	<section
		class="flex flex-col gap-4 rounded-3xl border border-slate-800 bg-slate-900/70 p-8 lg:flex-row lg:items-center lg:justify-between"
	>
		<div>
			<p class="text-sm tracking-[0.2em] text-slate-400 uppercase">Reportes · Oficiales</p>
			<h1 class="mt-2 text-4xl font-bold tracking-tight">Documentación oficial institucional</h1>
			<p class="mt-3 max-w-3xl text-sm text-slate-400">
				Generación y consulta de actas, libro matriz, nóminas, certificados y documentación
				requerida para inspección y archivo formal.
			</p>
		</div>

		<div class="flex gap-3">
			<a
				href="/reportes/oficiales/export.xlsx"
				class="rounded-2xl border border-slate-700 px-5 py-3 text-sm font-semibold transition hover:border-slate-500"
			>
				Exportar Excel
			</a>
			<a
				href="/reportes/oficiales/export.pdf"
				class="rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-slate-950 transition hover:scale-[1.02]"
			>
				Exportar PDF
			</a>
		</div>
	</section>

	<section class="grid gap-4 md:grid-cols-4">
		<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
			<p class="text-sm text-slate-400">Actas disponibles</p>
			<h2 class="mt-3 text-4xl font-bold">{data?.metrics?.records ?? 0}</h2>
		</div>

		<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
			<p class="text-sm text-slate-400">Libro matriz</p>
			<h2 class="mt-3 text-4xl font-bold">{data?.metrics?.matrix ?? 0}</h2>
		</div>

		<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
			<p class="text-sm text-slate-400">Nóminas</p>
			<h2 class="mt-3 text-4xl font-bold">{data?.metrics?.rosters ?? 0}</h2>
		</div>

		<div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
			<p class="text-sm text-slate-400">Certificados</p>
			<h2 class="mt-3 text-4xl font-bold">{data?.metrics?.certificates ?? 0}</h2>
		</div>
	</section>

	<section class="overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/70">
		<table class="w-full text-left">
			<thead class="border-b border-slate-800 bg-slate-900">
				<tr>
					<th class="px-6 py-4 text-sm font-semibold">Documento</th>
					<th class="px-6 py-4 text-sm font-semibold">Tipo</th>
					<th class="px-6 py-4 text-sm font-semibold">Período</th>
					<th class="px-6 py-4 text-sm font-semibold">Estado</th>
					<th class="px-6 py-4 text-right text-sm font-semibold">Acciones</th>
				</tr>
			</thead>
			<tbody>
				{#each documents as doc}
					<tr class="border-b border-slate-800 last:border-none">
						<td class="px-6 py-4 font-medium">{doc.name}</td>
						<td class="px-6 py-4">{doc.type}</td>
						<td class="px-6 py-4">{doc.period}</td>
						<td class="px-6 py-4">
							<span class="rounded-full border border-slate-700 px-3 py-1 text-xs">
								{doc.status}
							</span>
						</td>
						<td class="px-6 py-4 text-right">
							<a
								href={`/reportes/oficiales/${doc.id}/download`}
								class="rounded-xl border border-slate-700 px-3 py-2 text-sm transition hover:border-slate-500"
							>
								Descargar
							</a>
						</td>
					</tr>
				{/each}
			</tbody>
		</table>
	</section>
</div>
