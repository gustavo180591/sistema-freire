...
<div class="mx-auto max-w-7xl space-y-8">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
		<div>
			...
			<h1 class="text-3xl font-bold tracking-tight">Secretarios</h1>
			<p class="mt-2 text-sm text-slate-400">Administración del cuerpo de secretarios del instituto.</p>
		</div>
		<a
			href="/usuarios/nuevo"
	...
	<div class="grid gap-4 md:grid-cols-2">
		<div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
			<div class="text-2xl font-bold text-white">{data.secretaries.length}</div>
			<div class="text-sm text-slate-400">Total Secretarios</div>
		</div>
		<div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
			<div class="text-2xl font-bold text-green-400">
				{data.secretaries.filter((s: Secretary) => s.status === 'ACTIVE').length}
			</div>
	...
	<div class="rounded-3xl border border-slate-800 bg-slate-900/70">
		<table class="w-full table-fixed">
			...
			<tbody class="divide-y divide-slate-800">
				{#if filteredSecretaries.length === 0}
					<tr>
						<td colspan="5" class="px-3 py-8 text-center text-sm text-slate-400">
							No se encontraron secretarios que coincidan con "{searchQuery}"
						</td>
					</tr>
				{:else}
					{#each filteredSecretaries as secretary}
						<tr class="hover:bg-slate-800/30 transition-colors">
							...
							<td class="px-3 py-3 whitespace-nowrap text-sm text-slate-300 truncate" title={secretary.email}>
								{secretary.email}
							</td>
							<td class="px-3 py-3 whitespace-nowrap text-sm">
								<span class={getStatusColor(secretary.status)}>
									{getStatusText(secretary.status)}
								</span>
							</td>
		...
		<div class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
			<div class="bg-slate-900 rounded-3xl border border-slate-800 p-8 max-w-md w-full">
				<div class="flex items-center space-x-4 mb-6">
					...
					<div>
						<h2 class="text-xl font-bold text-white">Eliminar Secretario</h2>
						<p class="text-sm text-slate-400">Esta acción no se puede deshacer</p>
					</div>
				</div>

				<div class="bg-slate-800/50 rounded-2xl p-4 mb-6">
					<p class="text-white font-medium">{deletingSecretary.lastName}, {deletingSecretary.firstName}</p>
					<p class="text-sm text-slate-400">Email: {deletingSecretary.email}</p>
				</div>
