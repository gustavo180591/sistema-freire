...
<svelte:head>
	<title>Gestión de Secretarios | Paulo Freire</title>
</svelte:head>
							...
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">Comienza agregando el primer secretario al sistema</p>
									</div>
									{#if canManageSecretaries()}
										<a
											href="/usuarios/nuevo?type=SECRETARIA"
							...
							<td colspan="5" class="px-4 py-12 text-center">
								<div class="flex flex-col items-center gap-4">
									...
									<div class="space-y-1">
										...
										<p class="text-sm text-slate-400">No hay secretarios que coincidan con "{searchQuery}"</p>
									</div>
									<button
										onclick={clearSearch}
					...
					{:else}
						{#each filteredSecretaries as secretary}
							...
							<tr class="transition-colors hover:bg-slate-800/30">
								<td class="px-4 py-3">
									<div class="flex items-center gap-3">
										<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-cyan-500 to-blue-600 text-sm font-semibold text-white">
											{getInitials(secretary.firstName, secretary.lastName)}
										</div>
										<div class="min-w-0">
											<p class="text-sm font-medium text-white truncate" title={`${secretary.lastName}, ${secretary.firstName}`}>
								...
								<td class="px-4 py-3">
									<p class="text-sm text-slate-300 truncate" title={secretary.email}>{secretary.email}</p>
								</td>
								<td class="px-4 py-3">
								...
								<td class="px-4 py-3">
									<p class="text-sm text-slate-300">{new Date(secretary.createdAt).toLocaleDateString('es-AR')}</p>
								</td>
		...
		>
			<div class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
				<div class="flex items-start gap-4 mb-6">
					...
					<div class="flex-1">
						<h2 id="delete-modal-title" class="text-lg font-semibold text-white">Eliminar Secretario</h2>
						<p class="mt-1 text-sm text-slate-400">Esta acción no se puede deshacer. Se eliminará el rol de secretario del usuario.</p>
					</div>
				</div>

				<div class="mb-6 rounded-xl border border-slate-800 bg-slate-800/50 p-4">
					<p class="font-medium text-white">{deletingSecretary.lastName}, {deletingSecretary.firstName}</p>
