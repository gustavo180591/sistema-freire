<script lang="ts">
	...
	const icons: Record<string, string> = {
		'users-shield': 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z',
		'graduation-cap': 'M22 10v6M2 10l10-5 10 5-10 5z M12 12v9 M12 21l-7-3 M12 21l7-3',
		'chalkboard-teacher': 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z',
				...
				<div class="relative">
					...
					<div class="mt-8 flex flex-wrap gap-4">
						{#if !$page.data.user}
							<a
								href="/login"
								class="group inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 font-semibold text-white transition-all duration-200 hover:bg-indigo-500 hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900"
								aria-label="Ingresar al sistema"
							>
								Ingresar al sistema
								<svg class="h-4 w-4 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={icons['arrow-right']} />
								</svg>
							</a>
						{/if}
		...
		<div class="mt-16">
			...
			<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
				{#each modules as module}
					<a
						href={module.href || '#'}
						class="group relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70 p-6 transition-all duration-300 hover:border-slate-700 hover:bg-slate-900/90 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900"
						aria-label="{module.name}: {module.description}"
					>
						<div class="flex items-start gap-4">
							<div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl {module.bgColor}">
								<svg class="h-6 w-6 {module.color}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={icons[module.icon]} />
								</svg>
							</div>
